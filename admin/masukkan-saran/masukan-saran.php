...
$isiPesan = query("SELECT * FROM tb_pesan ORDER BY id DESC");
...
<body>
  ...
  <tbody>
    <?php $i = 1?>
    <?php foreach($isiPesan as $row) :?>
    <tr>
      <td><?= $i?></td>
      <td><?= $row['nama']?></td>
      <td><?= $row['email']?></td>
      <td><textarea name="textarea" id="textarea" cols="70" rows="10"><?= $row['isipesan']?></textarea></td>
      <td>
        <a href="./hapus_pesan.php?id=<?= $row['id']?>" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus pesan ini ?')">Hapus</a>
      </td>
    </tr>
    <?php $i++ ?>
    <?php endforeach ?>
